...
const blacknscholes = {
	...
	impliedVolatility:	(stock_price, strike, interest_rate, expiration_days, derivative_price, type)	=>	{
	  ...
	  // while (Math.round(theoretical_price,2) !== Math.round(derivative_price,2)	{
	    const d1 = blacknscholes.d1Term(stock_price, strike, volatility, interest_rate, expiration_days).toFixed(9);
	    const d2 = blacknscholes.d2Term(d1, volatility, expiration_days).toFixed(9);

	    theoretical_price = blacknscholes.price(stock_price, strike, interest_rate, expiration_days, d1, d2, 'call');
